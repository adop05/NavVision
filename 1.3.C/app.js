...
class SonarSynthesizer {
    constructor() {
        ...
        this.startDistance = 1.5;      // hazard label appears within this range
        this.criticalDistance = 0.30;  // fastest label-update rate at/below this
        this.slowestInterval = 500;    // ms, at startDistance (faster starting pace)
...
/**
 * Called by native code when SFSpeechRecognizer starts/stops actively
 * listening, so the button reflects real native state.
 * @param {boolean} isListening
 */
window.nativeVoiceListeningStateChanged = function (isListening) {
    const btn = document.getElementById('btn-voice-dest');
    if (!btn) return;
    btn.innerHTML = isListening
        ? '<i class="fa-solid fa-ear-listen"></i> Listening...'
        : '<i class="fa-solid fa-microphone"></i> Speak Destination';
};

/**
 * Called by native code with the final transcript once SFSpeechRecognizer
 * finishes listening successfully.
 * @param {string} transcript
 */
window.nativeVoiceRecognitionResult = function (transcript) {
    speech.logToConsole(`Heard: "${transcript}"`, "system-msg");
    fetchNavigation(state.currentLong, state.currentLat, transcript);
};

/**
 * Called by native code if speech recognition fails, is denied, or times
 * out with no speech detected.
 * @param {string} message
 */
window.nativeVoiceRecognitionError = function (message) {
    console.warn("Native speech recognition error:", message);
    speech.logToConsole(`Mic error: ${message}`, "hazard-msg");
};
...
// -------------------------------------------------------------
// 5b. Voice-Destination Routing (via Flask /get_route)
// -------------------------------------------------------------
//
// Unlike NavigationManager.fetchWalkingRoute() (which calls Mapbox
// Directions directly from the client using state.mapboxToken), this path
// sends a destination name to a Flask backend, which does server-side
// geocoding (including hardcoded campus landmark shortcuts) and Mapbox
// Directions, keeping the Mapbox token server-side. That backend needs to
// be reachable from the phone — configure its address in Settings under
// "Flask Backend Base URL" (same setting previously used for the now-
// removed YOLO camera backend). Run app.py on a machine on the same
// network as the phone (or behind an ngrok tunnel) and point this at it.
function fetchNavigation(startLong, startLat, destination) {
    fetch(`${state.backendUrl}/get_route`, {
        method: 'POST',
    ...
    .then(data => {
        ...
        // Map the Flask response shape to what the UI expects.
    ...
    .catch(err => {
        console.error(err);
        speech.logToConsole(`Routing Network Error — check Backend URL in Settings`, 'hazard-msg');
    });
}
...
function wireUIEvents() {
    ...
    // Speak Destination Button — now backed by native SFSpeechRecognizer
    // via the bridge, since Web Speech recognition isn't available in
    // WKWebView. See window.nativeVoiceRecognitionResult/Error/
    // ListeningStateChanged below for the native -> JS side of this.
    const btnVoiceDest = document.getElementById('btn-voice-dest');
    if (btnVoiceDest) {
        btnVoiceDest.addEventListener('click', () => {
            triggerHaptic(50);

            const startNativeListening = () => {
                sendNativeCommand('startVoiceRecognition');
            };

            // GPS lock: acquired on demand here rather than tied to the
            // camera/scanner toggle, since starting the LiDAR scanner no
            // longer touches getUserMedia or geolocation at all.
            ...
            if (!navigator.geolocation) {
                speech.speak("Location services aren't available on this device.");
                return;
            }

            speech.logToConsole("Acquiring GPS lock...", "system-msg");
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    speech.speak("Couldn't lock your location. Check location permissions.");
                },
                { enableHighAccuracy: true }
            );
        });
